...
func (l *wal) populateFilesMap() error {
	files, err := ioutil.ReadDir(l.db.opt.ValueDir)
	if err != nil {
...
func (l *wal) readIntentValue(vp valuePointer) ([]byte, error) {
	...
	l.mu.RUnlock()
	if lf == nil {
		return nil, errors.Errorf("wal file with id %d not found", vp.Fid)
	}
	...
	if !ok {
		return nil, errors.New("unable to decode wal transaction intent value")
	}
